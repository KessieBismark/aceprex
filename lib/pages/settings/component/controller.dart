...
class SettingsController extends GetxController {
  ...
  changePassword() async {
    ...
    if (skey.currentState!.validate()) {
      if (newP.text != cP.text) {
        Utils().showError("Password do not match!");
        return;
      }
      if (newP.text == currentP.text) {
        Utils().showError("New password cannot be the same as old password!");
        return;
      }
      try {
        isSave.value = true;
        ...
        if (jsonDecode(result) == 'true') {
          ...
          clearData();
          Utils.logOut();
        } else if (jsonDecode(result) == 'no match') {
          Utils().showError(
              "Current password does not match with what is in the system!");
          isSave.value = false;
        } else {
